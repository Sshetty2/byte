...
chmod 644 /home/sshetty/.ssh/authorized_keys

#sed -i  '' 's/^#AuthorizedKeysFile/^.*$/AuthorizedKeysFile \/home\/sshetty\/.ssh\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "# Created by Greg build process" >> /etc/ssh/sshd_config'

sh -c 'echo "AllowUsers sshetty root" >> /etc/ssh/sshd_config'
...
sed -i -e '/^Port/s/^.*$/Port 3333/' /etc/ssh/sshd_config
...
sh -c 'echo "sendername = security@greg-test-config-3" >> /etc/fail2ban/jail.local'
