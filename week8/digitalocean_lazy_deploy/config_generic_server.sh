...
#sed -i  '' 's/^#AuthorizedKeysFile/^.*$/AuthorizedKeysFile \/home\/<os_username>\/.ssh\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "AllowUsers <os_username> root" >> /etc/ssh/sshd_config'
